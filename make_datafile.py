...
END_TOKENS = ['.', '!', '?', '...', "'", "`", '"', dm_single_close_quote, dm_double_close_quote,
              ")", chinese_end]  # acceptable ways to end a sentence

# We use these to separate the summary sentences in the .bin datafiles
SENTENCE_START = '<s>'
SENTENCE_END = '</s>'

all_train_urls = "bin_data/filename_list/train.txt"
all_val_urls = "bin_data/filename_list/val.txt"
all_test_urls = "bin_data/filename_list/test.txt"

cnn_tokenized_stories_dir = "cnn_stories_tokenized"
dm_tokenized_stories_dir = "dm_stories_tokenized"
chinese_tokenized_stores_dir = "chinese_stories_tokenized"
...
num_expected_chinese_stories = 25180
...
def write_to_bin(url_file, out_file, makevocab=False):
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            if os.path.isfile(os.path.join(cnn_tokenized_stories_dir, s)):
            ...
            elif os.path.isfile(os.path.join(chinese_tokenized_stores_dir, s)):
                story_file = os.path.join(chinese_tokenized_stores_dir, s)
            else:
                print(
                ...
                print("Checking that the tokenized stories directories %s and %s, %s contain correct number of files..." % (
                cnn_tokenized_stories_dir, dm_tokenized_stories_dir, chinese_tokenized_stores_dir))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(chinese_tokenized_stores_dir): os.makedirs(chinese_tokenized_stores_dir)
    ...
    elif source.lower() == 'dm':
        check_num_stories(stories_dir, num_expected_dm_stories)
        tokenize_stories(stories_dir, dm_tokenized_stories_dir)
    elif source.lower() == 'chinese':
        check_num_stories(stories_dir, num_expected_chinese_stories)
        tokenize_stories(stories_dir, chinese_tokenized_stores_dir)
    else:
        raise TypeError('error type of file source.')
